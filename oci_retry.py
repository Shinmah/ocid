...
def write_summary(message):
    """Écrit un résumé visible directement sur la page du run GitHub Actions
    (section 'Summary'), sans avoir besoin d'ouvrir les logs."""
...
def main():
    ...
    while MAX_ATTEMPTS == 0 or attempt < MAX_ATTEMPTS:
        ...
        try:
            ...
            write_summary(f"## ✅ Instance créée avec succès\n\nOCID : `{response.data.id}`\n\nVa sur la console OCI (Compute > Instances) pour récupérer l'IP publique.")
            sys.exit(0)
    ...
    write_summary("## ⏳ Pas encore de capacité disponible\n\nAucune instance créée pendant ce run. Le prochain run automatique réessaiera dans 15 min.")
